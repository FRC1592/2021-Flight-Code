...
import frc.robot.lib1592.control.InterfacePID.AbsoluteTolerance;
import frc.robot.lib1592.control.InterfacePID.PercentageTolerance;
 ...
 * programmer.
 */
public abstract class PIDSubsystem extends SubsystemBase implements Sendable {
  /**
  ...
  /**
   * Instantiates a {@link PIDSubsystem} that will use the given constants.
   *
   * @param name  the name
   * @param constants  the PID constants
   */
  
  public PIDSubsystem(String name, PIDConstants constants) {
  ...
  public PIDSubsystem(PIDConstants constants) {
    m_controller = new PIDController(new BuilderPID(m_source, m_output, constants));
    addChild("PIDController", m_controller);
  }
  ...
  public void setAbsoluteTolerance(double t) {
	m_controller.setTolerance(new AbsoluteTolerance(m_controller, t));
  }
  ...
  public void setPercentTolerance(double p) {
    m_controller.setTolerance(new PercentageTolerance(m_controller, p));
  }
	 ...
	 * @param minimumInput
	 *            minimum input allowed by controller
	 * @param maximumInput
	 *            maximum input allowed by controller
	 */
	public synchronized void setContinuous(boolean continuous, double minimumInput, double maximumInput) {
		m_controller.setInputRange(minimumInput, maximumInput);
  ...
  /**
   * Reset the internal {@link PIDController}.
   */
  public void reset() {
    m_controller.reset();
  }
