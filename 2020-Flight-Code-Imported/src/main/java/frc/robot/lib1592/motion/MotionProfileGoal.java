...
public class MotionProfileGoal {
     ...
     * constraints.
     * 
     * ADJUST_ACCEL - If we absolutely do not want to pass the goal and are unwilling to violate the max_abs_vel
     * (for example, there is an obstacle in front of us - slam the brakes harder than we'd like in order to avoid
     * hitting it).
     * 
     * ADJUST_VELOCITY - If the final velocity is just a general guideline and not a hard performance limit, it's
    ...
    public static enum CompletionBehavior {
        ...
        // Only valid if the goal velocity is 0.0 (otherwise ADJUST_ACCEL will be used).
        OVERSHOOT,
    ...
    public MotionProfileGoal(MotionProfileGoal other) {
        this(other.pos, other.velMag, other.completion_behavior, other.pos_tolerance, other.vel_tolerance);
    }
    ...
    public MotionProfileGoal flipped() {
        return new MotionProfileGoal(-pos, velMag, completion_behavior, pos_tolerance, vel_tolerance);
    }
    ...
    public boolean atGoalState(TrajectoryPoint state) {
        return atGoalPos(state.pos) && (Math.abs(state.vel) < (velMag + vel_tolerance)
                || completion_behavior == CompletionBehavior.ADJUST_VELOCITY);
    }

    public boolean atGoalPos(double pos) {
        return MotionUtil.epsEquals(pos, this.pos, pos_tolerance);
    }
    ...
    protected void sanityCheck() {
        if (velMag > vel_tolerance && completion_behavior == CompletionBehavior.OVERSHOOT) {
        	System.out.println("Inconsistent Completion Behavior.  Won't OVERSHOOT nonzero velocity. Setting to ADJUST_ACCEL");
            completion_behavior = CompletionBehavior.ADJUST_ACCEL;
        }
    }

    @Override
    public String toString() {
        return "pos: " + pos + " (+/- " + pos_tolerance + "), max_abs_vel: " + velMag + " (+/- " + vel_tolerance
