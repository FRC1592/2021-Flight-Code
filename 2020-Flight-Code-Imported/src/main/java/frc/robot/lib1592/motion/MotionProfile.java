...
public class MotionProfile {
    ...
    /**
     * Get the interpolated TrajectoryPoint at any given time.
     * 
     * @param t	The time to query.
     * @return Empty if the time is outside the time bounds of the profile, or the resulting TrajectoryPoint otherwise.
     */
    public Optional<TrajectoryPoint> stateByTime(double t) {
        if (t < startTime() && t + MotionUtil.EPSILON >= startTime()) {
            return Optional.of(startState());
        }
        if (t > endTime() && t - MotionUtil.EPSILON <= endTime()) {
    ...
    /**
     * Get the interpolated TrajectoryPoint at any given time, clamping to the endpoints if time is out of bounds.
     * 
     * @param t		The time to query.
     * @return The TrajectoryPoint at time t, or closest to it if t is outside the profile.
     */
    public TrajectoryPoint stateByTimeClamped(double t) {
        if (t < startTime()) {
            return startState();
        } else if (t > endTime()) {
        	return endState();
        ...
        System.err.println("Reached invalid state in MotionProfile.stateByTimeClamped(t)");
        return TrajectoryPoint.kInvalidState;
    }

    /**
     * Get the interpolated TrajectoryPoint by distance (the "pos()" field of TrajectoryPoint). Note that since a profile may
     * reverse, this method only returns the *first* instance of this position.
     * 
     * @param pos	The position to query.
     * @return Empty if the profile never crosses pos or if the profile is invalid, or the resulting TrajectoryPoint
     *         otherwise.
     */
    public Optional<TrajectoryPoint> firstStateByPos(double pos) {
        for (MotionSegment s : mSegments) {
            if (s.containsPos(pos)) {
                if (MotionUtil.epsEquals(s.end().pos(), pos, MotionUtil.EPSILON)) {
     ...
     * 
     * @param initial_state
     *            The TrajectoryPoint to initialize to.
     */
    public void reset(TrajectoryPoint initial_state) {
        clear();
        mSegments.add(new MotionSegment(initial_state, initial_state));
    }
    
//    /**
//     * Shift the start time of every segment
//     * 
//     * @param dt time bias to apply to each segment
//     */
//    public void timeShift(double dt) {
//    	if (isEmpty()) return;
//    	double tStart = mSegments.get(0).start().t();
//    	for (Iterator<MotionSegment> iterator = mSegments.iterator(); iterator.hasNext();) {
//    		MotionSegment s = iterator.next();
//    		s.start().shiftTime(dt);
//    		s.end().shiftTime(dt);
//    	}
//    }
     ...
     * and set the final acceleration to zero.
     */
    public void finalize() {
    	consolidate();
    	//Set the acceleration of the last point to zero
    	TrajectoryPoint end = endState();
    	mSegments.get(mSegments.size() - 1).mEnd = new TrajectoryPoint(end.t,end.pos,end.vel,0);
    }
    ...
    public void appendControl(double acc, double dt) {
        ...
        TrajectoryPoint last_end_state = mSegments.get(mSegments.size() - 1).end();
        TrajectoryPoint new_start_state = new TrajectoryPoint(last_end_state.t(), last_end_state.pos(), last_end_state.vel(),acc);
     ...
     */
    public TrajectoryPoint startState() {
        if (isEmpty()) {
            return TrajectoryPoint.kInvalidState;
        }
...
//    /**
//     * Set the absolute start time of the Motion profile
//     * 
//     * @param time  start time in seconds
//     */
//    public void setStartTime(double time) {
//    	if (!mSegments.isEmpty()) {
//    		double deltaTime = time - mSegments.get(0).mStart.t();
//    		for (MotionSegment s : segments()) {
//    			s.shiftTime(deltaTime);
//    		}
//    	}
//    }
     ...
     */
    public TrajectoryPoint endState() {
        if (isEmpty()) {
            return TrajectoryPoint.kInvalidState;
        }
    ...
    /**
     * Does the motion profile contain the specified time
     * 
     * @param time  absolute time in seconds
     * @return  Does the motion profile contain the specified time
     */
