...
public class Vector2D {
	...
	/**
	 * 2D Cartesian Vector from Polar Description
	 *
	 * @param angle  angle in degrees
	 * @param mag  vector magnitude
	 * @return the cartesian 2D vector as described by the angle and magnitude
	 */
	public static Vector2D fromPolar(double angle, double mag) {
		double rad = Math.toRadians(angle);
		return new Vector2D(mag * Math.cos(rad), mag * Math.sin(rad));
	}
